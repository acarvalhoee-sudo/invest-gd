...
function NumInput({ value, onChange, prefix, suffix, step = 1, min = 0, max, error, ...props }: {
...
export default function StudyFormPage() {
  ...
  function validate(): boolean {
    ...
    if (u.potencia <= 0)      { e.potencia   = 'Potencia deve ser maior que zero'; setTab('usina') }
      ...
      <div className="p-6 lg:p-8 max-w-4xl mx-auto animate-fade-in">
        ...
        <div className="page-header mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="page-subtitle">{u.nomeEstudo || 'Preencha os dados do estudo'}</p>
            </div>
          </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="usina" className="space-y-6">
              ...
              <CardContent>
                <div className="form-grid-2">
                  <FieldGroup label="Nome do Estudo" required error={errors.nomeEstudo}>
                    <Input
                      placeholder="Ex.: CGH Rio Verde - Cenario Base"
                      value={u.nomeEstudo}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Fonte de energia, potencia e especificacoes operacionais</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="form-grid-3">
                  ...
                  <FieldGroup
                    label={`Potencia (${u.fonte === 'solar' ? 'kWp' : 'kW'})`}
                    required
                    hint={u.fonte === 'solar' ? 'Potencia de pico para usinas solares (kWp)' : 'Potencia instalada em kW'}
                  ...
                  <FieldGroup label="Tipo de GD">
                    <Select value={u.tipoGD} onValueChange={(v) => updateUsina({ tipoGD: v as TipoGD })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="GD I">GD I - ate 75 kW</SelectItem>
                ...
                <div className="form-grid-3">
                  <FieldGroup label="Fator de Capacidade (%)" hint="Relacao entre producao real e maxima teorica. Ex: CGH aprox 35%">
                ...
                <div className="form-grid-3">
                  <FieldGroup label="Degradacao Anual (%)" hint="Reducao anual de performance (ex: 0,5% para solar)">
                    <NumInput value={u.degradacaoAnual} onChange={(v) => updateUsina({ degradacaoAnual: v })} suffix="%" step={0.1} />
                  </FieldGroup>
                  <FieldGroup label="Disponibilidade (%)" hint="Percentual de tempo que a usina esta operacional">
                    <NumInput value={u.disponibilidade} onChange={(v) => updateUsina({ disponibilidade: v })} suffix="%" step={0.5} max={100} />
                  </FieldGroup>
                  <FieldGroup label="Perdas Eletricas (%)" hint="Perdas por cabeamento, inversores, transformadores">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Custos de aquisicao e implantacao</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="form-grid-2">
                  ...
                  <FieldGroup label="Custo da Obra de Rede (R$)" hint="Adequacao de rede, conexao, TUSD de acesso">
                ...
                <div className="rounded-xl bg-primary/5 border border-primary/20 p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-primary mt-1">{fmtCurrency(u.investimentoTotal)}</p>
                    </div>
                    <div>
            ...
            <div className="flex justify-end">
              <Button onClick={() => setTab('premissas')}>
                Proximo: Premissas <ChevronRight className="w-4 h-4" />
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="premissas" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Valores em R$/MWh e R$/kW - baseados na homologacao ANEEL vigente</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="form-grid-2">
                  ...
                  <FieldGroup label="TUSD (R$/MWh)" hint="Tarifa de Uso do Sistema de Distribuicao">
                    <NumInput value={p.tarifas.tusd} onChange={(v) => updateTarifas({ tusd: v })} prefix="R$" suffix="/MWh" step={10} />
                  </FieldGroup>
                  <FieldGroup label="TUSD Geracao (R$/MWh)" hint="Componente de uso da rede pelo gerador">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Aliquotas incidentes sobre a receita de energia injetada</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="form-grid-3">
                  <FieldGroup label="PIS (%)" hint="Programa de Integracao Social - aliquota padrao: 0,65%">
                    <NumInput value={p.impostos.pis} onChange={(v) => updateImpostos({ pis: v })} suffix="%" step={0.01} />
                  </FieldGroup>
                  <FieldGroup label="COFINS (%)" hint="Contribuicao para Financiamento da Seguridade Social - padrao: 3%">
                    <NumInput value={p.impostos.cofins} onChange={(v) => updateImpostos({ cofins: v })} suffix="%" step={0.01} />
                  </FieldGroup>
                  <FieldGroup label="ICMS (%)" hint="Imposto sobre Circulacao de Mercadorias - varia por estado">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Premissas Financeiras e Macroeconomicas</CardTitle>
                <CardDescription>Taxas de referencia para os calculos de rentabilidade (Fase 2)</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="form-grid-4">
                  ...
                  <FieldGroup label="IPCA (% a.a.)" hint="Indice de Precos ao Consumidor Amplo anual">
                    <NumInput value={p.financeiras.ipca} onChange={(v) => updateFinanceiras({ ipca: v })} suffix="%" step={0.1} />
                  </FieldGroup>
                  <FieldGroup label="SELIC (% a.a.)" hint="Taxa basica de juros - benchmark de comparacao">
                    <NumInput value={p.financeiras.selic} onChange={(v) => updateFinanceiras({ selic: v })} suffix="%" step={0.25} />
                  </FieldGroup>
                  <FieldGroup label="TMA (% a.a.)" hint="Taxa Minima de Atratividade - hurdle rate do investidor">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Despesas recorrentes ao longo do periodo de analise</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="form-grid-3">
                  <FieldGroup label="Manutencao (% inv./ano)" hint="Percentual anual sobre o investimento total">
                    <NumInput value={p.custos.manutencao} onChange={(v) => updateCustos({ manutencao: v })} suffix="%" step={0.1} />
                  </FieldGroup>
                  <FieldGroup label="Gestao (% receita)" hint="Percentual sobre a receita bruta anual">
                ...
                <div className="form-grid-2">
                  <FieldGroup label="Arrendamento (R$/ano)" hint="Custo de arrendamento de terra ou area">
                    <NumInput value={p.custos.arrendamento} onChange={(v) => updateCustos({ arrendamento: v })} prefix="R$" step={1000} />
                  </FieldGroup>
                  <FieldGroup label="Gestao Fixa Mensal (R$/mes)" hint="Valor fixo mensal de gestao operacional (corrigido pelo IPCA)">
            ...
            <div className="flex justify-between">
              ...
              <Button onClick={() => setTab('parametros')}>
                Proximo: Parametros <ChevronRight className="w-4 h-4" />
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="parametros" className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Periodo de Analise</CardTitle>
                <CardDescription>Define o horizonte temporal para projecao do fluxo de caixa</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="form-grid-3">
                  ...
                  <FieldGroup
                    label="Meses no 1o Ano"
                    hint="Informe quantos meses a usina operara no primeiro ano"
                  >
                    <Select
                ...
                <div className="rounded-xl bg-muted/50 border border-border p-5">
                  <p className="text-sm font-semibold text-foreground mb-4">Resumo do periodo de analise</p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                      { label: 'Duracao total', value: `${params.anoFinal - params.anoInicial + 1} anos` },
                      { label: 'Inicio', value: `${params.anoInicial}` },
                  ...
                  <div className="mt-4 p-3 bg-primary/5 rounded-lg border border-primary/20">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      <strong className="text-foreground">Regra aplicada:</strong> O primeiro ano tera{' '}
                      <strong>{params.mesesPrimeiroAno} meses</strong> de producao.
                      De {params.anoInicial + 1} a {params.anoFinal}, serao considerados 12 meses por ano.
                      Total: <strong>{params.mesesPrimeiroAno + (params.anoFinal - params.anoInicial) * 12} meses</strong> de operacao.
                    </p>
                  </div>
                </div>
              </CardContent>
